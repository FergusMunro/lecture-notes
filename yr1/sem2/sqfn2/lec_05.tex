...
\begin{proof} We first prove injectivity and then surjectivity.
  ...
  \item Fix $y > 0$. Then
    ...
    Hence, $\exp(- \frac{1}{y}) < y < \exp(y)$, so by the intermediate value theorem
    \[\exists c \in [ - \frac{1}{y}, y] \quad \text{such that} \quad \exp(c) = y.\]
  \end{itemize}
\end{proof}
...
\begin{definition}
Suppose $S \subseteq \mathbb{R}$. A point $c \in S$ is called an {\bf interior point} of $S$ if $\exists \delta > 0$ such that
...
\begin{definition}
  ...
  The number $f'(c) = \lim_{h \rightarrow 0} \frac{f(c + h) - f(c)}{h}$ is called {\bf the derivative} of $f$ at $c$.

  If $f$ is differentiable at every $c \in D$, we say $f$ is {\bf differentiable} on $D$.
\end{definition}
...
\begin{proof}
  ...
  \begin{align*}
    \lim_{x \rightarrow c}\left(f(x) - f(c)\right) &= \lim_{c \rightarrow c}\left(\frac{f(x) - f(c)}{x - c}(x - c)\right) \\
    &= \lim_{x \rightarrow c} \frac{f(x) - f(c)}{x - c} \lim_{x \rightarrow c} (x - c) \\ 
    &= f'(c)(0) = 0
  .\end{align*} 
  so $\lim_{x \rightarrow c} f(x) = f(c)$, so $f$ is continuous at $c$.
\end{proof}
...
\begin{proof}
    ...
    \begin{itemize}
      \item First, it is easy to show that
        \[(fg)(x + h) - (fg)(x) = \left(f(x + h) - f(x)\right)g(x + h) + f(x)\left(g(x + h) - g(x)\right).\]
        Now, taking the limit of this expression divided by $h$ gives
        \[\lim_{h \rightarrow 0} \frac{\left(f(x + h) - f(x)\right)g(x + h)}{h} + \frac{f(x)\left(g(x + h) - g(x)\right)}{h}.\]
        Since $g$ is differentiable and therefore continuous, $\lim_{h \rightarrow 0}g(x + h) = g(x)$, so by the algebra of function limits the product rule is obtained.
      \item We only need to prove that $\left( \frac{1}{g}\right)'(c) = \frac{-g'(c)}{(g(c))^2}$, and can then apply this with the product rule to prove the quotient rule. We also need to use the continuity of $g$ to prove $g(x + h) \not = 0$ less than some distance $\delta$ away. Finally
        \begin{align*}
          \lim_{h \rightarrow 0} \frac{ \left(\frac{1}{g}\right)(x + h) - \left(\frac{1}{g}\right)(x)}{h} &= \lim_{h \rightarrow 0} \frac{g(x) - g(x + h)}{hg(x)g(x + h)} \\
          &= \lim_{h \rightarrow 0} - \frac{g'(x)}{g(x)g(x + h)}
        \end{align*}
        as required.
    \end{itemize}
